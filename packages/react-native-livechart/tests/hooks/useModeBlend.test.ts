...
import { useModeBlend } from "../../src/hooks/useModeBlend";

describe("useModeBlend", () => {
  it("starts at 0 when isCandle is false", async () => {
  ...
  it("starts at 1 when isCandle is true", async () => {
    const { result } = await renderHook(() => {
      const lineOpacity = useSharedValue(1);
  ...
  it("lineGroupOpacity is 1 when line mode and lineOpacity is 1", async () => {
    const { result } = await renderHook(() => {
      const lineOpacity = useSharedValue(1);
      return useModeBlend(false, lineOpacity);
    });
  ...
  it("candleGroupOpacity is 0 when line mode", async () => {
    const { result } = await renderHook(() => {
      const lineOpacity = useSharedValue(1);
      return useModeBlend(false, lineOpacity);
    });
  ...
  it("lineGroupOpacity is 0 when candle mode", async () => {
    const { result } = await renderHook(() => {
      const lineOpacity = useSharedValue(1);
      return useModeBlend(true, lineOpacity);
    });
  ...
  it("candleGroupOpacity is 1 when candle mode and lineOpacity is 1", async () => {
    const { result } = await renderHook(() => {
      const lineOpacity = useSharedValue(1);
      return useModeBlend(true, lineOpacity);
    });
  ...
  it("scales both opacities by lineOpacity", async () => {
    const { result } = await renderHook(() => {
  ...
  it("both opacities are 0 when lineOpacity is 0", async () => {
    const { result } = await renderHook(() => {
  ...
  it("triggers animation when isCandle toggles", async () => {
  ...
  it("returns to 0 when switching back to line mode", async () => {
    const { result, rerender } = await renderHook(
      (props: { isCandle: boolean }) => {
        const lineOpacity = useSharedValue(1);
        return useModeBlend(props.isCandle, lineOpacity);
