...
The experimental frame-loop controls are available on `LiveChart`:

```ts
interface LiveChartFrameStats {
  frames: number;    // active engine callbacks
  published: number; // callbacks that changed a tracked engine value
  skipped: number;   // callbacks whose tracked engine values were unchanged
}
...
See [Scroll interaction](/guides/scroll-interaction) for the parent-scroll gate
and the counter's measurement limits.
...
interface ReferenceLine {
  ...
  extendToStart?: boolean;       // flat-extend the first value left; default true
...
interface LineConfig {
  ...
  color?: string | SharedValue<string>; // line color override; defaults to palette
                    //   accent. A SharedValue animates the stroke and the built-in
                    //   selection dot on the UI thread; a custom selection dot,
                    //   segment base, area-dot tint, and threshold rest color then
                    //   use the palette line color
...
interface GradientConfig {
  ...
  colors?: string[] | SharedValue<string[]>; // explicit gradient stops (top →
                          //   bottom), ≥ 2 entries; overrides topOpacity/
                          //   bottomOpacity when provided. A SharedValue animates
                          //   the stops on the UI thread; keep ≥ 2 entries in it
  positions?: number[];   // optional stop positions (0..1, ascending), same
                          //   length as colors; ignored if the lengths differ
                          //   (with animated colors, applied as given)
}
...
interface ThresholdConfig {
  ...
  extendToStart?: boolean;    // series: extend first value flat to the window start; default true
...
interface YAxisConfig {
  // Label side; reserve left label width with insets.left. Default "right".
  side?: "left" | "right";
