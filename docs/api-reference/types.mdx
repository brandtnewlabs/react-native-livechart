...
interface ChartSegment {
  ...
  recolorLine?: boolean;     // participate in scrub-focus; default true
  lineColor?: string;        // de-emphasis color when NOT focused; default: color
                             //   (an alpha color, e.g. "rgba(154,160,166,0.4)", fades the line)
  lineColors?: string[];     // ≥2 → de-emphasis gradient; precedence over lineColor
  active?: boolean;          // force-focus this segment without scrubbing; default false
  // Optional translucent band (independent of the line recolor):
  color?: string;            // resting band fill; default: accent color
  opacity?: number;          // resting band opacity; default 0.06
  highlightColor?: string;   // band fill when focused; default: color
  highlightOpacity?: number; // band opacity when focused; default 0.16
  // Divider + label:
  divider?: boolean;         // dashed vertical line at `from`; default false
  dividerColor?: string;     // default: color
  label?: string;            // caption — shown only when the divider is shown
