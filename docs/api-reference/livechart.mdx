...
<ParamField body="line" type="LineConfig">
  ...
  `color` also accepts a `SharedValue<string>` to animate the stroke and the
  built-in scrub selection dot. See [Line & area](/guides/line-and-area#animating-the-line-and-fill-color)
  for an example and the [`LineConfig`](/api-reference/types#config-objects)
...
<ParamField body="gradient" type="boolean | GradientConfig" default="true">
  ...
  accent color. `colors` also accepts a `SharedValue<string[]>` to animate the
  fill stops. See [Line & area](/guides/line-and-area#animating-the-line-and-fill-color)
  for an example; keep at least two stops in the shared value.
</ParamField>
...
<ParamField body="referenceLines" type="ReferenceLine[]">
  ...
  time band). Series lines work in both line and candle mode. Their first value
  extends to the visible window start unless `extendToStart` is `false`, and their
  last value extends to the live edge unless `extendToNow` is `false`. Form-A lines
...
<ParamField body="threshold" type="ThresholdConfig">
  ...
  horizontal `labelPosition` and first/last visible `labelAnchor`. Set
  `extendToStart: false` when the threshold did not exist before its first point.
  Supersedes `line.color`/`colors` and segment recoloring for the main stroke while set. See
...
<ParamField body="yAxis" type="boolean | YAxisConfig" default="true">
  Value-axis grid + labels. Set `{ side: "left" }` and reserve label space with
  `insets.left` for left-side labels; the default side is `"right"`. Pass `{ count }` for a fixed number of evenly-spaced
...
<ParamField body="isFrameLoopActive" type="SharedValue<boolean>">
  Runtime gate for continuous frame work. Set it to `false` while a parent list
  scrolls; data can still update, and the chart catches up when it becomes
  `true`. The chart and its gestures stay mounted. Unlike `static`, this can be
  changed on the UI thread without a React render. Experimental; see
  [Scroll interaction](/guides/scroll-interaction).
</ParamField>

<ParamField body="debugFrameStats" type="SharedValue<LiveChartFrameStats>">
  Optional development counter with `frames`, `published`, and `skipped` totals
  for active engine callbacks. A gated frame is not counted. `published` tracks
  changes to engine values, not every possible Skia redraw. The counter itself
  writes a SharedValue each frame, so omit it from production profiling.
  Experimental; see [Scroll interaction](/guides/scroll-interaction).
</ParamField>
