...
`yAxis={{ side: "left" }}` also places multi-series Y-axis labels in the gutter
reserved by `insets.left`; right-side series value labels remain unchanged.
...
<ParamField body="dot" type="boolean | MultiSeriesDotConfig" default="true">
  Per-series live dots: `radius`, `ring`, opt-in static `glow`, `color`,
  ...
  `dot={{ show: false }}` still hides them.)
</ParamField>
