...
<LiveChart
  data={data}
  value={value}
  threshold={{ value: breakEven, fill: true, extendToStart: false }}
/>;
```

- By default, the series is **clamped to its first/last value** outside its own time range. So
...
- **`extendToStart: false`** opts out of the backward extension when the benchmark
  did not exist yet (for example, break-even before the first trade). Left of the
  first point the stroke keeps its plain line color; the fill and marker are
  absent, the threshold does not affect range fitting, and a left-positioned
  badge follows the marker start while remaining inside the plot.
