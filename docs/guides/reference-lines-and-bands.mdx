...
Lines support labels, dashes (`intervals`), stroke opacity, and excluding their value from the axis
range (`excludeFromRange`). Bands can use separate `fillColor` / `fillOpacity` and
...
`extendToStart: false` to begin the path at its first point, or `extendToNow: false` to stop the
path and its label at the final point. A series contributes its minimum and maximum to automatic
Y-range fitting unless `excludeFromRange` is set.
