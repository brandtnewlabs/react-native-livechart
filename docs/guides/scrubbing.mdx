...
`scrubAction` **coexists** with `scrub`/`onScrub`: with no reticle placed, a drag scrubs as usual;
once a reticle is locked, dragging adjusts it. (Set `scrub={{ dimOpacity: 1 }}` to keep working-order
