...
line stroke itself via a horizontal gradient — an alpha-reduced `lineColor`
...
const segments: ChartSegment[] = [
  ...
  { to: t1, lineColor: "rgba(154,160,166,0.4)" },
  // regular session, with a dashed divider + label at its start
  { from: t1, to: t2, lineColor: "rgba(154,160,166,0.4)", divider: true, label: "Regular" },
  // after-hours: t2 → live edge; `active` force-focuses it without a scrub
  { from: t2, lineColor: "rgba(154,160,166,0.4)", divider: true, label: "After hours" },
];

<LiveChart data={data} value={value} segments={segments} />;
```

Each segment can de-emphasize with a solid `lineColor` (use an alpha color to
fade) or a `lineColors` gradient, force itself focused with `active`, and mark its
edge with a dashed `divider` + `label` (the label shows only when the divider
does). A translucent background band is also available (`color` / `opacity` and
`highlightColor` / `highlightOpacity`) independently of the line recolor — set the
opacities to `0` for a pure line-focus look. See
[`ChartSegment`](/api-reference/types#chartsegment) for every field, and
